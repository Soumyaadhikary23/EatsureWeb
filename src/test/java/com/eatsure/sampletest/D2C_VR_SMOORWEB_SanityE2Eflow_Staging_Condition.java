...
public class D2C_VR_SMOORWEB_SanityE2Eflow_Staging_Condition {

	public WebDriver driver;
	Driver driver1;
	SoftAssert as;
	public static final Logger log = LogManager.getLogger(D2C_VR_SMOORWEB_SanityE2Eflow_Staging_Condition.class.getName());
	...
	public void initialize(@Optional("chrome") String browser) throws IOException {
		...
		driver.get(Driver.prop.getProperty("urlStagingSMOOR"));
	}
...
//    	landing.enterLocation(Driver.prop.getProperty("location"));
		landing.enterLocation(ReadPropertyFile.get("smoorlocation"));
...
//		locality.addDynemiComboOfFourProductcondition(driver);
	...
	public void selectionOfRestaurant() {
		...
		as = new SoftAssert();
		LocalityPage locality = PageFactory.initElements(driver, LocalityPage.class);
		locality.navigateToBrandProduction(driver);
	...
	@Test(enabled = true, priority = 8, description = "selectionOfNormalandDynamicProductwithCustomizationFromPLPExploreAllDishes")
...
//		plp.clicksecondExploreAll(driver);
		...
		// Commented as we are getting different types of products
//		brand.addDynemiComboOfTwoProductcondition(driver);
...
//		as.assertEquals(plp.verifyPresenceofVegorNonVegIcon(driver, ProductListingPage.nonVegIcon,"nonVeg:- "), false,
...
//		as.assertEquals(plp.verifyProductListingPageDrpdownSelectItem(), true, "Product Listing Pagedropdown selection not validated");
        ...
        //Right now we are commenting the dynemicComboAddition as it is handled for two products
//		brand.addDynemiComboOfTwoProductcondition(driver);
