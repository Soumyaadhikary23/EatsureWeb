...
import com.eatsure.utility.CommonUtility;
...
public class D2C_VR_ESWEB_SanityE2Eflow_Payment_Staging_PAYLATER {

	public static final Logger log = LogManager.getLogger(D2C_VR_ESWEB_SanityE2Eflow_Payment_Staging_PAYLATER.class.getName());
	...
	@Test(enabled = true, priority = 2, description = "selectionOfRestaurant")
	...
	@Test(enabled = true, priority = 3, description = "selectionOfNormalProductwithCustomizationFromRestaurantPage")
	...
	@Test(enabled = true, priority = 4, description = "loginfromCartpage")
	public void loginfromCartPage() throws InterruptedException {
		ExtentReport.createTest("loginfromCartpage");
		CartPage cart = PageFactory.initElements(driver, CartPage.class);
		cart.enterMobileNumber(ReadPropertyFile.get("MobilenumberPayments"));
		Thread.sleep(1000);
		cart.clickLoginwithOTP();
		CommonUtility comm = new CommonUtility();
		String otp = comm.otp_api_call("https://api-staging25.faasos.io/v1/get_otp_by_phone_number/9255552525.json");
	...
	@Test(enabled = true, priority = 5, description = "addAddressfromCartPage")
	...
	@Test(enabled = true, priority = 6, description = "visibilityOfPaymentPage")
	...
	@Test(enabled = true, priority = 7, description = "selectionOfPaymentMode")
	public void selectionOfPaymentMode() {
		ExtentReport.createTest("selectionOfPaymentMode");
		PaymentsPage payments = PageFactory.initElements(driver, PaymentsPage.class);
		payments.verifyChoosePaymentText(driver);
		payments.clickSimplButton(driver);
		payments.verifySimplCard(driver);
		payments.verifyAmount();
		log.info("Selected Simpl paylater pay mode");
	}

	@Test(enabled = true, priority = 8, description = "proceedToPay")
	public void proceedToPay() {
		ExtentReport.createTest("proceedToPay");
		PaymentsPage payments = PageFactory.initElements(driver, PaymentsPage.class);
		payments.clickPayButton(driver);
		log.info("Used Simpl paylater mode to place an order");
	}

	@Test(enabled = true, priority = 9, description = "trackingOrderPage")
