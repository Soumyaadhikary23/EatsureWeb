...
public class D2C_VR_OSWEB_SanityE2Eflow_Staging_Condition {

	public WebDriver driver;
	Driver driver1;
	SoftAssert as;
	public static final Logger log = LogManager.getLogger(D2C_VR_OSWEB_SanityE2Eflow_Staging_Condition.class.getName());
	...
	public void initialize(@Optional("chrome") String browser) throws IOException {
		...
		driver.get(Driver.prop.getProperty("urlStagingOS"));
	}
...
//    	landing.enterLocation(Driver.prop.getProperty("location"));
		...
		ValidateTitle.verifyTitle(driver, "Order Pizzas Online from Oven Story Pizza (Official)");
		log.info("Successfully Landing Page title verified");
	}	
	
	@Test(enabled = true, priority = 1, description = "selectionOfNormalProductwithCustomizationFromRestaurantPage")
	...
	@Test(enabled = true, priority = 2, description = "selectionOfDynamicProductwithCustomizationFromRestaurantPage")
	...
	@Test(enabled = true, priority = 4, description = "selectionOfDynamicProductwithCustomizationFromPDP")
...
//		cart.clickVerifyOTP();
	...
	public void selectionOfPaymentMode() {
		...
		payments.clickCODButtonforBB(driver);
		payments.verifyCODCard();
		payments.verifyAmount();
		log.info("Selected COD pay mode");
	}

	@Test(enabled = true, priority = 11, description = "proceedToPay")
	...
	@Test(enabled = true, priority = 12, description = "trackingOrderPage")
