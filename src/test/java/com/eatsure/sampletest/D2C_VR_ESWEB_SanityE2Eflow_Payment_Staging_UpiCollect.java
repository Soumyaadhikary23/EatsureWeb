...
public class D2C_VR_ESWEB_SanityE2Eflow_Payment_Staging_UpiCollect {

	public static final Logger log = LogManager.getLogger(D2C_VR_ESWEB_SanityE2Eflow_Payment_Staging_UpiCollect.class.getName());
	...
	public void selectionOfPaymentMode() throws Exception{
		ExtentReport.createTest("selectionOfPaymentMode");
		PaymentsPage payments = PageFactory.initElements(driver, PaymentsPage.class);
		payments.verifyChoosePaymentText(driver);
		payments.clickUpiCollectButton(driver);
		payments.verifyUpiCollectCard(driver);
		log.info("Selected UPI collect pay mode");
	}

	@Test(enabled = false, priority = 8, description = "proceedToPay")
	public void proceedToPay() {
		ExtentReport.createTest("proceedToPay");
		PaymentsPage payments = PageFactory.initElements(driver, PaymentsPage.class);
		payments.clickPayButton(driver);
		log.info("Used upi collect mode to place an order");
	}
